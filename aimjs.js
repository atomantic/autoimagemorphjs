...
var fs = require("fs"),
  PNG = require("pngjs/lib/png").PNG; // https://github.com/lukeapage/pngjs

function loadpng(filename) {
  //   return PNG.sync.read(fs.readFileSync(`${__dirname}/output/${filename}`));
...
function newpng2(w, h) {
  return new PNG({ filterType: -1, width: w, height: h });
}
...
function tridraw(
  ...
  for (var i = 0; i < itarg; i++) {
    ...
    inpb1 = [
      incoords1[0][0] + inbdx1 * iprog,
      incoords1[0][1] + inbdy1 * iprog,
    ];
    inpa1 = [
      incoords1[1][0] + inadx1 * iprog,
      incoords1[1][1] + inady1 * iprog,
    ];
    inpb2 = [
      incoords2[0][0] + inbdx2 * iprog,
      incoords2[0][1] + inbdy2 * iprog,
    ];
    inpa2 = [
      incoords2[1][0] + inadx2 * iprog,
      incoords2[1][1] + inady2 * iprog,
    ];
    ...
    for (var j = 0; j < Math.floor(sl * subpixel); j++) {
      ...
      inps1 = [
        inpb1[0] + (inpa1[0] - inpb1[0]) * jprog,
        inpb1[1] + (inpa1[1] - inpb1[1]) * jprog,
      ];
      inps2 = [
        inpb2[0] + (inpa2[0] - inpb2[0]) * jprog,
        inpb2[1] + (inpa2[1] - inpb2[1]) * jprog,
      ];
      (ins1 = [0, 0, 0, 0]), (ins2 = [0, 0, 0, 0]);
      // RGBA values if on image
      if (
        inps1[0] >= 0 &&
        inps1[0] < inpng1.width &&
        inps1[1] >= 0 &&
        inps1[1] < inpng1.height
      ) {
      ...
      if (
        inps2[0] >= 0 &&
        inps2[0] < inpng2.width &&
        inps2[1] >= 0 &&
        inps2[1] < inpng2.height
      ) {
      ...
      if (
        ...
        outpng.data[idx1 + 1] = Math.floor(
          ins1[1] * (1 - alfa) + ins2[1] * alfa
        );
        outpng.data[idx1 + 2] = Math.floor(
          ins1[2] * (1 - alfa) + ins2[2] * alfa
        );
        outpng.data[idx1 + 3] = Math.floor(
          ins1[3] * (1 - alfa) + ins2[3] * alfa
        );
      }
    ...
    for (var i = 0; i <= n; i++) {
      ...
      if (j > 0 && j < m && i > 0 && i < n) {
        ...
        for (var y = 0; y < ch; y++) {
          for (var x = 0; x < cw; x++) {
            idx =
              (png.width * Math.floor(y + j * ch - ch / 2) +
                Math.floor(x + i * cw - cw / 2)) *
              4;
            ...
            if (
              (tc === fc && Math.hypot(x - cw / 2, y - ch / 2) < fd) ||
              tc > fc
            ) {
...
function drawfeaturepoints(png, fpts, col) {
  for (var j = 0; j < fpts.length; j++) {
    for (var i = 0; i < fpts[j].length; i++) {
      var idx =
        (png.width * Math.floor(fpts[j][i][1]) + Math.floor(fpts[j][i][0])) * 4;
      png.data[idx] = col[0];
...
function getframe(png1, png2, outpng, fpts1, fpts2, alfa, subpixel) {
  for (var j = 0; j < fpts1.length - 1; j++) {
    for (var i = 0; i < fpts1[j].length - 1; i++) {
      // iterate on feature point grid, each quad is drawn as 2 triangles
      tridraw(
        png1,
        png2,
        outpng,
        ...
        [
          [
            fpts1[j][i][0] * (1 - alfa) + fpts2[j][i][0] * alfa,
            fpts1[j][i][1] * (1 - alfa) + fpts2[j][i][1] * alfa,
          ],
          [
            fpts1[j][i + 1][0] * (1 - alfa) + fpts2[j][i + 1][0] * alfa,
            fpts1[j][i + 1][1] * (1 - alfa) + fpts2[j][i + 1][1] * alfa,
          ],
          [
            fpts1[j + 1][i][0] * (1 - alfa) + fpts2[j + 1][i][0] * alfa,
            fpts1[j + 1][i][1] * (1 - alfa) + fpts2[j + 1][i][1] * alfa,
          ],
      ...
      tridraw(
        ...
        [
          ...
          [
            fpts1[j + 1][i + 1][0] * (1 - alfa) + fpts2[j + 1][i + 1][0] * alfa,
            fpts1[j + 1][i + 1][1] * (1 - alfa) + fpts2[j + 1][i + 1][1] * alfa,
          ],
          [
            fpts1[j + 1][i][0] * (1 - alfa) + fpts2[j + 1][i][0] * alfa,
            fpts1[j + 1][i][1] * (1 - alfa) + fpts2[j + 1][i][1] * alfa,
...
try {
  ...
  for (var i = 2; i < process.argv.length; i++) {
    if (o.hasOwnProperty(process.argv[i])) {
      k = process.argv[i];
    } else {
      o[k].unshift(process.argv[i]);
    }
  }
  anim(
    o["-i"].reverse(),
